// FH1 4D5309C9 pixel shader 2B83D3E3C1FCEE3C only.
...
struct Fh1BinaryAlphaMaskPixel {
...
float SampleAlpha(float2 coordinate) {
  ...
  float alpha = xe_textures_2d[texture_index].SampleGrad(
      xe_samplers[xe_descriptor_indices.y], float3(coordinate, 0.0), dx, dy).w;
  if (sign == 2u) alpha = alpha * 2.0 - 1.0;
  else if (sign == 3u) alpha = DecodeGammaSigned(alpha);
  const int exponent = SignExtend((xe_fetch[0].w >> 13) & 0x3Fu, 6);
  return alpha * asfloat(uint(exponent + 127) << 23);
}

Fh1BinaryAlphaMaskPixel main(float4 uv : TEXCOORD0,
                             float4 alpha_scale : TEXCOORD1,
                             float4 position : SV_Position) {
  const float threshold = SampleAlpha(uv.xy * c[0].xy) * alpha_scale.x - c[1].x;
  clip(threshold - c[1].y);

  const float alpha = 0.0;
  clip(AlphaTest(alpha) ? 1.0 : -1.0);
  Fh1BinaryAlphaMaskPixel output;

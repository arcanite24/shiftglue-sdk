// FH1 shadow mask 26C4FD34AECBE4DE / 0000400300000001.
...
float4 main(float4 projected:TEXCOORD0,float4 position:SV_Position):SV_Target {
 ...
 precise float ps=0;bool p0=false;
  ...
  if(p0) { // 44
    precise float4 v = Mul(r0.xxxx, c[16].xxxx) + c[15].zzzz;
    r0.z = v.z;
  }
  if(p0) { // 45
  ...
  { // 70
    precise float4 v = -(abs(r0)).xyyy + c[13].xxxx;
    r0.xy = v.xy;
  }
  { // 71
  ...
  { // 72
    precise float4 v = saturate(Mul(-r0.xxxx, c[13].yyyy) + c[15].zzzz);
    r0.x = v.x;
  }
  { // 73

// FH1 shadow mask 8418C40F121D7EA7 / 0000400300000001.
#define FH1_SHADOW_CONSTANT_COUNT 16
...
float4 main(float4 projected:TEXCOORD0, float4 position:SV_Position):SV_Target {
 ...
 precise float ps=0;
  { // 4
    precise float4 v = Mul((abs(r1)).xxyy, c[8].xxyy);
  ...
  { // 5
    precise float4 v = Mul(r1.xxxx, c[15].xyzz);
    r2.xyz = v.xyz;
  }
  { // 6
  ...
  { // 7
    precise float4 v = r0.xwww + c[15].yyyy;
    r1.xw = v.xw;
  }
  ...
  { // 11
    precise float4 v = Mul(c[9].zzzz, c[6].yyyy);
  ...
  { // 17
    precise float4 v = (MulS((r2.xyyy).x, (c[9].xyyy).x) + MulS((r2.xyyy).y, (c[9].xyyy).y)) + (c[13].yyyy).x;
    r1.x = v.x;
  }
  { // 18
    precise float4 v = r1.xyyy + r1.wzzz;
    precise float s = max((c[15].zz).x, (c[15].zz).y);
    r0.xy = v.xy;
    r2.w = s;
    ps = s;
  }
  { // 19
    precise float4 v = Dot4(c[10].zxyw, r2.zxyw);
    precise float s = c[9].w + r0.x;
    r0.w = v.w;
    r0.z = s;
    ps = s;
  }
  { // 20
    precise float4 v = Mul(r0.yyyy, c[14].zzzz) + c[15].yyyy;
    r0.x = v.x;
  }
  { // 21
    precise float4 v = r0.zzww + c[7].zzww;
    r0.yz = v.yz;
  }
  { // 22
    precise float4 v = -r0.yyzz + c[15].wwww;
    r0.yz = v.yz;
  }
  { // 23
    precise float4 v = -(abs(r0)).yyzz + c[7].yyyy;
    r0.yz = v.yz;
  }
  { // 24
    precise float4 v = min(r0.yyyy, r0.zzzz);
  ...
  { // 25
    precise float4 v = Mul(r0.yxxx, c[14].yxxx);
    r0.xy = v.xy;
  }
  { // 26
    precise float4 v = saturate(r0.xxxx + c[14].wwww);
  ...
  { // 29
    precise float4 v = Mul(r1.xyyx, c[12]) + r0.yzyz;
    r3.xyzw = v.xyzw;
  }
  { // 30
    precise float4 v = Mul(r1.xyxy, c[11].xzwy) + r0.yyzz;
    r1.xyzw = v.xyzw;
  }
  ...
  { // 37
    precise float4 v = Dot4(r1.wzxy, c[15].wwww);

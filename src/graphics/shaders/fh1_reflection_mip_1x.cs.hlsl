// Reflection mip filter: endian 8-in-32, tiled R10G10B10A2, symmetric 1x.
...
uint Offset(uint2 p, uint pitch) {
  return TiledOffset(p.x,p.y,pitch);
}
...
void main(uint3 id : SV_DispatchThreadID) {
  ...
  // Each aligned 2x2 block has offsets 0,4,16,20 in the checked unscaled layout.
  // Avoid the observed optimized-FXC miscompile of repeated neighbor-coordinate expressions.
  uint input=source_offset+id.z*source_face_stride+Offset(id.xy*2,max(32u,source_side));
  ...
  pixels.Store(destination_offset+id.z*destination_face_stride+Offset(id.xy,max(32u,destination_side)),Swap(packed));
}

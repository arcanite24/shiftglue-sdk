// FH1 4D5309C9 world shader 984DBF6AF14DBEBD only.
// Direct form of the title's 25 guest instructions. This is the two-UV
// sibling of fh1_world_lit.vs.hlsl and deliberately uses the existing Xenos
// root bindings.
...
Fh1WorldLitUv2Vertex main(uint vertex_id : SV_VertexID) {
  Fh1WorldLitUv2Vertex output;
  ...
  precise float2 texture_coordinate_0 =
      Fh1Mul2(local_uv.x, c[13].xy) + c[15].xy;
  output.texture_coordinates.xy =
      Fh1Mul2(local_uv.y, c[14].xy) + texture_coordinate_0;
  precise float2 texture_coordinate_1 =
      Fh1Mul2(local_uv.x, c[16].xy) + c[18].xy;
  output.texture_coordinates.zw =
      Fh1Mul2(local_uv.y, c[17].xy) + texture_coordinate_1;
  ...
  output.selector = 0.0;
  output.selector.xyz = -abs(local_uv.x) > c[19].x ? 1.0 : 0.0;
  return output;
}

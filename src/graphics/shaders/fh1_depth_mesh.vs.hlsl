// FH1 4D5309C9 depth-only mesh vertex shaders only.
// The four captured programs are identical except for their fixed vertex stride.

#ifndef FH1_DEPTH_STRIDE_BYTES
#error FH1_DEPTH_STRIDE_BYTES must match the captured FH1 vertex program
#endif

ByteAddressBuffer xe_shared_memory_srv : register(t0);
#ifndef FH1_DEPTH_OWNED_GEOMETRY
...
#ifdef FH1_DEPTH_OWNED_GEOMETRY
  // Root SRVs have no hardware bounds. Preserve the shared-memory fallback's
  // per-component zero result at its 512 MiB boundary.
...
float3 Fh1Mul3(float3 a, float3 b) {
  return float3(Fh1Mul(a.x, b.x), Fh1Mul(a.y, b.y),
                Fh1Mul(a.z, b.z));
}
...
float4 main(uint vertex_id : SV_VertexID) : SV_Position {
  ...
  const uint address =
      (xe_fetch[47].z & ~3) + index * FH1_DEPTH_STRIDE_BYTES;
  const float3 local_position = asfloat(Load3(address, vertex_endian));
  const float4 model_position =
      float4(Fh1Mul3(local_position, c[0].xyz) + c[1].xyz,
             abs(local_position.x) >= 0.0 ? 1.0 : 0.0);
  const float4 view_position =
      float4(Fh1Dot4(c[2], model_position) - c[10].w,
             Fh1Dot4(c[3], model_position) - c[11].w,
             Fh1Dot4(c[4], model_position) - c[12].w,
             Fh1Dot4(c[5], model_position));
  // Projection sums w, z, x, y; the model transform above sums z, x, y, w.
  precise float4 position = float4(Fh1Dot4(c[6].zxwy, view_position.zxwy),
                           Fh1Dot4(c[7].zxwy, view_position.zxwy),
                           Fh1Dot4(c[8].zxwy, view_position.zxwy),
                           Fh1Dot4(c[9].zxwy, view_position.zxwy));
  if (!(xe_system[0].x & 8)) {

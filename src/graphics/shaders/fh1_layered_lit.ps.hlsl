// FH1 4D5309C9 pixel shader 9584B309533EF6C9 only.
...
Fh1LayeredLitPixel main(float4 uv : TEXCOORD0,
                        centroid float4 base_color : TEXCOORD1,
                        centroid float4 lighting0 : TEXCOORD2,
                        centroid float4 lighting1 : TEXCOORD3,
                        float4 position : SV_Position) {
  const float4 texel = SampleTexture(uv.xy);
  const float3 linear_color =
      texel.xyz * c.z * lighting1.xyz * lighting0.xyz * base_color.w +
      base_color.xyz;
  const float alpha = texel.w * lighting1.w;
  clip(AlphaTest(alpha) ? 1.0 : -1.0);

  Fh1LayeredLitPixel output;
